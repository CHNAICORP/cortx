...
const DEFAULT_SYSTEM = [
  ...
  "",
  "== 联网搜索策略 ==",
  "搜索前先思考：用户到底需要什么信息？最少搜几次能搞定？",
  "搜索效率原则：",
  "  1. 先搜一次，看搜索结果的摘要(snippet)是否已包含答案",
  "  2. 摘要够用 → 直接回答；不够 → 用 web_fetch 读取 1 个最相关的页面",
  "  3. 仍然不够 → 换关键词再搜一次（最多 2-3 次搜索）",
  "  4. 简单问题（股价、天气、定义）通常 1 次搜索即可，不需要 web_fetch",
  "⚠️ 每次搜索约需 10-15 秒，不要无意义地反复搜索。搜索前先看已有结果是否足够。",
  "⚠️ 避免抓取 news.google.com、duckduckgo.com 等易超时站点，优先选专业站点。",
  "",
  "== 技能系统 (Skills) ==",
